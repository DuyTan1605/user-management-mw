...
JVM_JMX_ARGS="-Dcom.sun.management.jmxremote.host=$JVM_JMX_HOST \
...
if [ "x$JVM_HTTP_PROXY_HOST" != "x" ]; then
JVM_ARGS="$JVM_ARGS -Dhttp.proxyHost=$JVM_HTTP_PROXY_HOST"
fi

if [ "x$JVM_HTTP_PROXY_PORT" != "x" ]; then
JVM_ARGS="$JVM_ARGS -Dhttp.proxyPort=$JVM_HTTP_PROXY_PORT"
fi

if [ "x$JVM_HTTPS_PROXY_HOST" != "x" ]; then
JVM_ARGS="$JVM_ARGS -Dhttps.proxyHost=$JVM_HTTPS_PROXY_HOST"
fi

if [ "x$JVM_HTTPS_PROXY_PORT" != "x" ]; then
JVM_ARGS="$JVM_ARGS -Dhttps.proxyPort=$JVM_HTTPS_PROXY_PORT"
fi

if [ "x$JVM_SOCK_PROXY_HOST" != "x" ]; then
JVM_ARGS="$JVM_ARGS -DsocksProxyHost=$JVM_SOCK_PROXY_HOST"
fi

if [ "x$JVM_SOCK_PROXY_PORT" != "x" ]; then
JVM_ARGS="$JVM_ARGS -DsocksProxyPort=$JVM_SOCK_PROXY_PORT"
fi
...
if [ "x$JVM_XMN" == "x$JVM_XMX" ] && [ "x$JVM_XMN" != "x" ]; then
echo "Bad config, -Xmn($JVM_XMN) is equal to -Xmx($JVM_XMX). This causes high CPU by GC. Stop launching."
exit
fi
